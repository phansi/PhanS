...
function activateaccount($email){
    ...
    $sql = 'UPDATE `account` set activated = 1 where `email` = '.'"'.$email.'"'.'';
    $data = array();
    $stmt = $dbCon->prepare($sql);
    $stmt->execute();
}
...
function removecard1($serialnumber){
    require ('connection.php');
    $sql = 'DELETE FROM `recharge_history` where seri = '.$serialnumber.'';
    $stmt = $dbCon->prepare($sql);
    $stmt->execute();
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        $card1[] = array(
            ...
            'money'                         => $row['money'],
                 'status'                        => $row['status'],
            'numberofcards'                 => $row['numberofcards'],
            'Data_expiration'                 => $row['Data_expiration'],
        );
    }
